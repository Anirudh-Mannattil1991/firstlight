...
import { userJourneyA, userJourneyB, userJourneyC } from "@/data/seedData";
import { Calendar, Plus, Trash2, ChevronRight } from "lucide-react";

export default function PregnancyTracker() {
  const { t } = useLanguage();
  const [selectedJourney, setSelectedJourney] = useState("A");
  ...
  const journeyLabels = {
    A: "Urban - High Risk (32 weeks)",
    B: "Rural - Normal (28 weeks)",
    C: "Teenage - Irregular Care (20 weeks)",
  };

  const pregnancy = pregnancies[selectedJourney as keyof typeof pregnancies];

  const handleAddAppointment = async () => {
    if (!appointmentDate) return;
    // In real app, this would persist to DB
    setAppointmentDate("");
    ...
    <div className="min-h-screen bg-gradient-to-br from-rose-50 to-white">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          {Object.entries(journeyLabels).map(([key, label]) => (
            <button
              key={key}
            ...
            >
              ...
              <p className="text-sm opacity-75">
                {pregnancies[key as keyof typeof pregnancies].currentWeek} weeks
              </p>
            </button>
          ))}
        </div>
