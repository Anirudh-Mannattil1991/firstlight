...
export default function NutritionTracker() {
  ...
  const nutritionData = {
    A: {
      label: "Urban - High Risk Pregnancy",
      logs: nutritionLogsUserA,
    },
    B: {
      label: "Rural - Normal Pregnancy",
      logs: nutritionLogsUserB,
    },
    C: {
      label: "Teenage - Irregular Care",
      logs: nutritionLogsUserC,
    },
  };

  const currentData = nutritionData[selectedUser as keyof typeof nutritionData];
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          {Object.entries(nutritionData).map(([key, data]) => (
            ...
            >
              ...
              <p className="text-sm opacity-75">{data.logs.length} log entries</p>
            </button>
          ))}
        </div>
